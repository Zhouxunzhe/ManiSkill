...
def load_pretrained_unet(run_name, tag):
    """加载预训练UNet模型的参数"""
    checkpoint = torch.load(f"runs/{run_name}/checkpoints/{tag}.pt")
    pretrained_agent = Agent(envs, args).to(device)
    pretrained_agent.load_state_dict(checkpoint["agent"])
    # 返回UNet的状态字典
    return pretrained_agent.noise_pred_net.state_dict()


def extract_unet_components(pretrained_unet_dict):
    """从预训练UNet的状态字典中提取down_path和up_path组件的参数"""
    down_path_dict = {}
    up_path_dict = {}

    # 根据参数名称区分down_path和up_path组件
    for key, value in pretrained_unet_dict.items():
        if key.startswith("down_modules") or key.startswith("mid_modules"):
            down_path_dict[key] = value
        elif key.startswith("up_modules") or key.startswith("final_conv"):
            up_path_dict[key] = value

    return down_path_dict, up_path_dict


def compute_parameter_distance(generated_params, pretrained_params):
    """计算生成的参数与预训练参数之间的差异"""
    total_loss = 0.0
    param_count = 0
    param_losses = {}
    ...
    # 遍历生成的参数字典
    for gen_key, gen_value in generated_params.items():
        # 找到匹配的预训练参数
        for pre_key, pre_value in pretrained_params.items():
            # 参数名称可能不完全一致，使用部分匹配
            if any(segment in pre_key for segment in gen_key.split('.')):
                if gen_value.shape == pre_value.shape:
                    # 形状相同，直接计算MSE
                    param_loss = F.mse_loss(gen_value, pre_value)
                    param_losses[f"{gen_key}-{pre_key}"] = param_loss
                    ...
                    try:
                        # 对于不匹配的批次维度，取生成参数的均值
                        if gen_value.dim() > pre_value.dim() and gen_value.shape[0] > 1:
                            adjusted_gen_value = gen_value.mean(dim=0)
                            if adjusted_gen_value.shape == pre_value.shape:
                                param_loss = F.mse_loss(adjusted_gen_value, pre_value)
                                param_losses[f"{gen_key}-{pre_key}"] = param_loss
                                total_loss += param_loss
                                param_count += 1
                                break
    ...
    # 如果找到了匹配的参数，返回平均loss
    if param_count > 0:
        return total_loss / param_count, param_losses
    else:
        # 如果没有找到匹配的参数，返回0loss
...
class Agent(nn.Module):
    ...
    def compute_loss(self, data_batch, current_step=0, total_steps=100000, pretrained_unet_dict=None):
        """计算训练损失，包括diffusion loss和参数约束loss"""
        ...
        # 参数约束损失
        down_param_loss = torch.tensor(0.0, device=self.device)
        up_param_loss = torch.tensor(0.0, device=self.device)
        param_losses_dict = {}

        if pretrained_unet_dict is not None:
            # 提取预训练模型的down_path和up_path参数
            pretrained_down_params, pretrained_up_params = extract_unet_components(pretrained_unet_dict)

            # 计算参数约束损失
            down_param_loss, down_losses = compute_parameter_distance(down_path_params, pretrained_down_params)
            up_param_loss, up_losses = compute_parameter_distance(up_path_params, pretrained_up_params)

            # 添加到损失字典中
            param_losses_dict.update(down_losses)
            param_losses_dict.update(up_losses)
        ...
        # 计算总参数损失
        param_loss = down_param_loss + up_param_loss
        ...
        if pretrained_unet_dict is not None:
            # 参数约束损失权重设为0.1，可以根据实际情况调整
            param_weight = 0.1
            total_loss = total_loss + param_weight * param_loss
        ...
        if pretrained_unet_dict is not None:
            self.loss_dict['down_param_loss'] = down_param_loss.item()
            self.loss_dict['up_param_loss'] = up_param_loss.item()
            self.loss_dict['total_param_loss'] = param_loss.item()
...
def load_ckpt(run_name, tag):
    checkpoint = torch.load(f"runs/{run_name}/checkpoints/{tag}.pt")
    loaded_agent = OriginalAgent(original_envs, original_args).to(device)
    loaded_agent.load_state_dict(checkpoint["agent"])
    loaded_ema_agent = OriginalAgent(original_envs, original_args).to(device)
    loaded_ema_agent.load_state_dict(checkpoint["ema_agent"])
    return loaded_agent, loaded_ema_agent


if __name__ == "__main__":
    from train_rgbd import Agent as OriginalAgent
    from train_rgbd import Args as OriginalArgs
    ...
    # 创建评估环境
    original_args.env_id = "PickCubeYCB-v1"
    original_args.num_eval_envs = 1

    original_envs = make_eval_envs(
        original_args.env_id,
        original_args.num_eval_envs,
        original_args.sim_backend,
        env_kwargs,
        other_kwargs,
        video_dir=f"runs/{run_name}/videos" if original_args.capture_video else None,
    ...
    pretrained_run_name = "PickCubeYCB-single_stack_red_cube_blue_cube-diffusion-80"  # 替换为预训练模型的run_name
    pretrained_tag = "best_eval_success_at_end"
    pretrained_agent, _ = load_ckpt(pretrained_run_name, pretrained_tag)
    pretrained_unet_dict = pretrained_agent.noise_pred_net.state_dict()
    ...
    for iteration, data_batch in enumerate(train_dataloader):
        ...
        total_loss = agent.compute_loss(
            data_batch, current_step=iteration, total_steps=args.total_iters,
            pretrained_unet_dict=pretrained_unet_dict
        )
