...
class VideoMAEEncoder(nn.Module):
    def __init__(self, output_dim=512, dropout_rate=0.3, device=None):
        super().__init__()
        # Store device
        self.device = device
        ...
        # VideoMAE configuration parameters
        self.image_size = 224  # Required input size for VideoMAE
        self.patch_size = self.model.config.patch_size  # Typically 16
        self.tubelet_size = self.model.config.tubelet_size  # Typically 2
        self.hidden_size = self.model.config.hidden_size  # videomae-base: hidden_size = 768

        # 3D Conv for processing features after VideoMAE
        self.conv3d = nn.Sequential(
            nn.Conv3d(self.hidden_size, 256, kernel_size=(3, 1, 1), padding=(1, 0, 0)),
            nn.BatchNorm3d(256),
        ...
        # Move conv3d to specified device
        if self.device is not None:
        ...
        # Move task_fc to specified device
        if self.device is not None:
            self.task_fc = self.task_fc.to(self.device)
        ...
        # Move task_classifier to specified device
        if self.device is not None:
            self.task_classifier = self.task_classifier.to(self.device)

        self.feature_norm = nn.LayerNorm(128)
        if self.device is not None:
            self.feature_norm = self.feature_norm.to(self.device)
        ...
        # Move contrastive_projector to specified device
        if self.device is not None:
            self.contrastive_projector = self.contrastive_projector.to(self.device)
    ...
    def forward(self, video):
        ...
        for b in range(batch):
            batch_tensors = []
            for t in range(T):
                frame = video[b, t]  # (128, 128, C)
                if frame.dtype != torch.uint8:
                    frame = (frame * 255) if frame.max() <= 1.0 else frame
                frame = frame.permute(2, 0, 1)
                frame_tensor = resize_transform(frame)  # (C, 224, 224)
                batch_tensors.append(frame_tensor)
            batch_tensor = torch.stack(batch_tensors, dim=0)  # (T, C, 224, 224)
            output_tensors.append(batch_tensor)

        video_input = torch.stack(output_tensors, dim=0)  # (batch, T, C, 224, 224)
        video_input = video_input.permute(0, 1, 3, 4, 2)  # (batch, T, 224, 224, C)

        video_features = []
        for i in range(batch):
            single_video = video_input[i]
            single_video = single_video.permute(0, 3, 1, 2)
            # HERE: we need to move frames to the device where model is
            if self.device is not None:
                video_frames = [frame.to(self.device) for frame in single_video]
            else:
                video_frames = [frame for frame in single_video]
            ...
            video_feature = outputs.last_hidden_state[:, 0, :]  # (1, hidden_size)
            video_features.append(video_feature)

        features = torch.cat(video_features, dim=0)
        hidden_size = features.shape[1]

        # Reshape for 3D convolution (add dummy temporal and spatial dims)
        features_3d = features.view(batch, hidden_size, 1, 1, 1)

        # Apply 3D convolution
        features = self.conv3d(features_3d)  # [batch, 128, 1, 1, 1]
        features = features.squeeze(-1).squeeze(-1)  # [batch, 128, 1]
        features = features.squeeze(-1)  # [batch, 128]

        # Apply feature normalization
        global_features = self.feature_norm(features)  # [batch, 128]

        # Extract task-specific features
        task_features = self.task_fc(global_features)  # [batch, output_dim]
        ...
        # Generate task classification logits
...
class Agent(nn.Module):
    def __init__(self, env: VectorEnv, args: Args, device):
        ...
        # Video encoder - PASS THE DEVICE HERE
        self.video_encoder = VideoMAEEncoder(output_dim=ftask_dim, device=device).to(device)
    ...
    def compute_loss(self, data_batch):
        ...
        #     return loss
        #
        # contrastive_loss = info_nce_loss(ftask, labels)
