...
class RealmanArmMotionPlanningSolver:
    ...
    def setup_planner(self):
        ...
        right_planner.set_base_pose(np.hstack([self.base_pose.p, self.base_pose.q]))
        left_planner.set_base_pose(np.hstack([self.base_pose.p, self.base_pose.q]))
        return right_planner
    ...
    def move_to_pose_with_screw(
        ...
        if self.grasp_pose_visual is not None:
            self.grasp_pose_visual.set_pose(pose)
        pose = sapien.Pose(p=pose.p , q=pose.q)
        ik_status, ik_results = self.planner.IK(
            np.concatenate([pose.p, pose.q]),
            self.robot.get_qpos().cpu().numpy()[0],
        )
        print(ik_status)
...
def build_realman_gripper_grasp_pose_visual(scene: ManiSkillScene):
