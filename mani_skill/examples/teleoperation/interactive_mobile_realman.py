...
class Args:
    ...
    obs_mode: str = "none"
    robot_uid: Annotated[str, tyro.conf.arg(aliases=["-r"])] = "mobile_realman"
    """The robot to use. Robot setups supported for teleop in this script are mobile_realman"""
...
def solve(env: BaseEnv, debug=False, vis=False):
    ...
    planner = RealmanArmMotionPlanningSolver(
        env,
        debug=debug,
        vis=vis,
    ...
    gripper_open = True
    def select_realman_hand():
        viewer.select_entity(sapien_utils.get_obj_by_name(env.agent.robot.links, "r_hand_base_link")._objs[0].entity)
    select_realman_hand()
    ...
    while True:
        ...
        if viewer.window.key_press("h"):
            ...
            u: move the mobile_realman hand up
            j: move the mobile_realman hand down
            arrow_keys: move the mobile_realman hand in the direction of the arrow keys
            n: execute command via motion planning to make the robot move to the target pose indicated by the ghost mobile_realman arm
        ...
        if execute_current_pose:
            # z-offset of end-effector gizmo to TCP position is hardcoded for the mobile_realman robot here
            if env.unwrapped.robot_uids == "mobile_realman":
                result = planner.move_to_pose_with_RRTConnect(transform_window._gizmo_pose * sapien.Pose([0, 0, 0.1]), dry_run=True)
