...
class Args:
    ...
    """The robot to use. Robot setups supported for teleop in this script are realman"""
...
def solve(env: BaseEnv, debug=False, vis=False):
    ...
    while True:
        ...
        if viewer.window.key_press("h"):
            ...
            u: move the realman hand up
            j: move the realman hand down
            arrow_keys: move the realman hand in the direction of the arrow keys
        ...
        if execute_current_pose:
            # z-offset of end-effector gizmo to TCP position is hardcoded for the realman robot here
            if env.unwrapped.robot_uids == "realman":
