...
python -m examples.baselines.diffusion_policy.train_rgbd --env-id FoldSuitcase-v1 \
  --demo-path /home/engram/zhouxunzhe/ManiSkill/demos/FoldSuitcase-v1/motionplanning/train_fold_all_1k.rgbd.pd_joint_delta_pos.physx_cpu.h5 \
  --control-mode "pd_joint_delta_pos" --shader rt --num-demos 200 --max_episode_steps 500 --total_iters 50000 --batch_size 256 \
  --log_freq 1000 --eval_freq 3000 --save_freq 1000 --num_eval_episodes 100 --num_eval_envs 1 --visual_encoder "siglip" \
...
python -m examples.baselines.diffusion_policy.train_rgbd_lan --env-id "FoldSuitcase-v1" \
  ...
  --obs_mode "rgb+depth" --lan_encoder "encoder_ffn" --language_condition_type "adapter" --sparse_steps 4 \
  --exp_name "Fold-all-shared-encoder_ffn-adapter-500" --prompt "Fold laptop, box, suitcase"