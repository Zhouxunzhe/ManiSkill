...
python -m mani_skill.examples.motionplanning.panda.run \
  --traj-name "pick_red_cube_plate_stable.rgbd.pd_joint_pos.cpu" -e "PickCubeYCB-v1" -n 500 \
  --obs-mode rgbd --only-count-success -b cpu --shader rt --num-procs 1 

# Replay data to get delta pos
python -m mani_skill.trajectory.replay_trajectory \
  --traj-path /home/engram/zhouxunzhe/ManiSkill/demos/PickCubeYCB-v1/motionplanning/pick_red_cube_plate_stable.rgbd.pd_joint_pos.cpu.h5 \
...
python -m examples.baselines.diffusion_policy.train_rgbd --env-id PickCubeYCB-v1 \
  ...
  --obs_mode rgb+depth --exp_name PickCubeYCB-pick_red_cube_plate_stable-hyper_net-500 \
